...
A location could be discarded from the output target by using the @discard@ keyword with value set to @true@.

<pre>
location myId discard = "true" "http://download.eclipse.org/releases/juno/" {
    ...
}
</pre>
...
<pre>
location discard = "true" "http://download.eclipse.org/releases/juno/" {
    ...
}
</pre>

It is really usefull when you use it with a "variable call":#TPDWithVariables:


<pre>
location discard = ${someVariableTrueOrFalse} "http://download.eclipse.org/releases/juno/" {
    ...
}
</pre>

h3(#TPDWithVariables). TPD file with variables
...
* @location discard state@

<pre>
location discard = ${someVariableTrueOrFalse} "someURL" {
    ...
}
</pre>
