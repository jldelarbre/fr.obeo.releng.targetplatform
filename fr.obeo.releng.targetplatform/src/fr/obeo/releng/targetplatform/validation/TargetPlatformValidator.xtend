...
class TargetPlatformValidator extends AbstractTargetPlatformValidator {
	...
	def checkOneEnvironment(TargetPlatform tp) {
		...
		if (envList.size > 1) {
			envList.tail.forEach[
				warning('''Environment definition should not be split accros the file.''', tp, TargetPlatformPackage.Literals.TARGET_PLATFORM__CONTENTS, tp.contents.indexOf(it), CHECK__ENVIRONMENT_UNICITY)
			]
		}
	}
	...
	def checkOneOptions(TargetPlatform tp) {
		...
		if (envList.size > 1) {
			envList.tail.forEach[
				warning('''Options definition should not be split accros the file.''', tp, TargetPlatformPackage.Literals.TARGET_PLATFORM__CONTENTS, tp.contents.indexOf(it), CHECK__OPTIONS_UNICITY)
			]
		}
	}
