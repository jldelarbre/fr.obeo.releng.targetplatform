...
h4. Outline view with variables

Variables are shown in outline view with dedicated icons according to their origin (local, imported or diamondly imported).
...
In the example _pom.xml_ above, the _maven_ property @targetPlatform.application.args@ holds the command line arguments. You can define them (tpd filenames shall be first, the two last parameters are optional and case incensitive):
...
<pre>
myTpd.tpd --useEnv --override var1=someValue var2=otherValue
